...
export default function ResultPage() {
  ...
  const regionTexts = useMemo(() => Object.fromEntries(results.map((result) => [result.regionId, result.status === 'succeeded' ? (drafts[result.id] ?? '') : ''])), [drafts, results]);
    ...
    <div className="document-page">
      ...
      <main className="page-width result-screen">
        ...
        <div className="result-workspace">
          <aside className="result-preview-pane" aria-label={t.document.resultCropTitle}><div className="result-preview-pane__header"><span>{t.document.resultCropTitle}</span>{selectedLine && <span className="font-mono">{(selectedLine.readingOrder ?? 0) + 1}</span>}</div>{page && previewUrl && regions.length > 0 ? <LineOverlay imageUrl={previewUrl} imageWidth={page.width} imageHeight={page.height} regions={regions} regionTexts={regionTexts} emptyText={t.document.resultMissing} selectedRegionId={selectedLine?.regionId} onSelectRegion={(regionId) => setSelectedId(results.find((line) => line.regionId === regionId)?.id ?? null)} className="result-page-preview" /> : <div className="result-preview-empty">{t.document.resultMissing}</div>}</aside>
